  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (e.status === 403 || e.code === "ACCESS_DENIED") {
        setServerError(
          "Yêu cầu phiên đăng nhập Google hợp lệ. Nếu bạn chưa đăng nhập hoặc phiên đã hết hạn, vui lòng bấm nút 'Đăng nhập bằng Google' bên dưới."
        );
      } else {
        setServerError(e.message || "Thiết lập mật khẩu thất bại. Vui lòng thử lại.");
      }
    }
  };
      ...
      >
        ...
        <div className="relative space-y-6">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 backdrop-blur-md text-white text-xs font-semibold">
            ...
            <span>Kích hoạt bảo mật tài khoản SAGA</span>
          </div>

          <p className="text-4xl lg:text-5xl font-extrabold text-white leading-tight">
            Thiết lập mật khẩu <span className="text-white/70">lần đầu</span>
          </p>
          <p className="text-white/70 text-lg leading-relaxed max-w-md">
            Đặt mật khẩu cục bộ để kích hoạt đầy đủ quyền hạn truy cập đồ án, lớp học và các tính năng nâng cao.
          </p>
        </div>
        ...
        <div className="w-full max-w-sm space-y-5">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-primary mb-1">
              ...
              <h1 className="text-xl font-bold text-foreground">Thiết lập mật khẩu</h1>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed">
              Tài khoản Google của bạn cần đặt mật khẩu lần đầu để hoàn tất xác thực trên hệ thống SAGA.
            </p>
          </div>

          {user?.email && (
            <div className="flex items-center gap-2.5 p-2.5 rounded-xl bg-primary/5 border border-primary/20 text-xs">
              <UserCheckIcon className="size-4 text-primary shrink-0" />
              <div className="min-w-0">
                <p className="font-semibold text-foreground truncate">{user.fullName || user.email}</p>
          ...
          {success ? (
            <div className="py-8 flex flex-col items-center justify-center text-center space-y-3 rounded-2xl bg-emerald-500/5 border border-emerald-500/20 p-6 animate-in zoom-in-95">
              ...
              <p className="font-bold text-foreground text-sm">Đã thiết lập mật khẩu thành công!</p>
              <p className="text-xs text-muted-foreground">Đang tự động chuyển hướng vào không gian làm việc...</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-3.5" noValidate>
              ...
              <div className="space-y-1">
                <Label htmlFor="confirm-password" className="text-xs font-semibold">
                  Xác nhận lại mật khẩu <span className="text-destructive">*</span>
                </Label>
                <div className="relative">
                  <Input
                    id="confirm-password"
              ...
              {serverError && (
                <div className="space-y-2 p-3 rounded-xl bg-destructive/10 text-destructive text-xs animate-in fade-in-0">
                  ...
                  {!isAuthenticated && (
                    <Button
                      ...
                      className="w-full text-xs font-bold gap-2 mt-1.5 border-destructive/30 hover:bg-destructive/10 text-destructive cursor-pointer"
                    >
                      <GoogleIcon />
                      <span>Đăng nhập lại bằng Google (FPT / FE)</span>
                    </Button>
                  )}
                </div>
              )}
          ...
          <p className="text-center text-[11px] text-muted-foreground">
            Sau khi lưu mật khẩu, bạn có thể đăng nhập bằng tài khoản Google hoặc Email/Password bất cứ lúc nào.
          </p>
        </div>
      </div>
    </div>
