          ...
          {unifiedItems.map((item) => {
            ...
            return (
              <div key={`commit-${commit.id}`} className="relative group">
                <div className="absolute -left-6 top-1.5 size-5 rounded-full bg-card border border-purple-500/40 text-purple-600 dark:text-purple-400 flex items-center justify-center shadow-2xs">
                  <GitCommitIcon className="size-2.5" />
                </div>

                <div className="p-3 rounded-xl border border-border/70 bg-card hover:border-purple-500/40 transition-all shadow-2xs space-y-1.5">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-mono text-xs font-bold text-purple-600 dark:text-purple-400 bg-purple-500/10 border border-purple-500/20 px-1.5 py-0.5 rounded-md">
                      {shortSha}
                    </span>
