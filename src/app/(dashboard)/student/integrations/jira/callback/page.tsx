...
function JiraCallbackContent() {
  ...
  const [errorMessage, setErrorMessage] = useState(() =>
    hasParams ? "" : "Thiếu mã xác thực (code) hoặc state từ Atlassian OAuth."
  );

  const callbackMutation = useJiraOAuthCallback();
  ...
  useEffect(() => {
    ...
    callbackMutation
      ...
      .catch((err: Error) => {
        if (!isMounted) return;
        setStatus("error");
        setErrorMessage(err.message || "Xác thực OAuth thất bại từ máy chủ.");
      });
        ...
        <CardContent className="p-0 space-y-4">
          {status === "processing" && (
            <div className="space-y-3">
              ...
              <h3 className="text-base font-bold text-foreground">Đang xác thực Atlassian Jira OAuth...</h3>
          ...
          {status === "success" && (
            <div className="space-y-3">
              ...
              <h3 className="text-base font-bold text-foreground">Liên kết Jira thành công!</h3>
              <p className="text-xs text-muted-foreground">
