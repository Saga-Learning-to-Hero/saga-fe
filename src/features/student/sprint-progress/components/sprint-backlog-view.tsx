      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl bg-muted/40 border border-border/60">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground font-mono">
            ({backlogSprints.length} sprints · {productBacklogIssues.length} backlog tasks)
          </span>
        </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2 shrink-0">
                      ...
                      <>
                        ...
                        {sprint.status === "ACTIVE" && (
                          <Button
                            ...
                            className="h-8 text-xs font-bold rounded-xl gap-1 bg-primary hover:bg-primary/90 text-primary-foreground cursor-pointer px-3"
                          >
                    ...
                    <Button
                      ...
                      className="h-8 text-xs font-semibold rounded-xl gap-1 cursor-pointer hover:bg-primary/10 hover:text-primary hover:border-primary/40"
                    >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="text-base font-bold text-foreground flex items-center gap-2">
                    <LayersIcon className="w-4.5 h-4.5 text-purple-600 dark:text-purple-400" />
                    <span>Backlog (Product Backlog)</span>
                  </h3>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Danh sách công việc tồn đọng chưa gán vào Sprint. Kéo thả lên Sprint phía trên để phân bổ thực hiện.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 shrink-0">
              <div className="text-right text-xs font-mono mr-2">
                ...
                <strong className="text-foreground font-bold">{backlogTotalSP}</strong>
              ...
              >
                <PlusIcon className="w-3.5 h-3.5" />
                <span>+ Thêm việc vào Backlog</span>
              </Button>
            </div>
          </div>
        </CardHeader>
        ...
        {!isBacklogCollapsed && (
          <CardContent className="p-0 max-h-[500px] overflow-y-auto divide-y divide-border/60 scrollbar-thin scrollbar-thumb-border hover:scrollbar-thumb-muted-foreground/30">
            {productBacklogIssues.length === 0 ? (
              ...
              productBacklogIssues.map((issue) => {
                const canDrag = isTeamLeader || issue.assignee.studentCode === currentUserStudentCode;
