...
export function ClassMemberTab({ courseId }: ClassMemberTabProps) {
  ...
  const filteredMembers = useMemo(() => {
    return members.filter(m => {
      ...
      const studentCodeStr = (m.studentCode || m.studentId || "").toLowerCase();
      const matchesSearch = !searchQuery ||
        m.fullName.toLowerCase().includes(searchLower) ||
        m.email.toLowerCase().includes(searchLower) ||
        studentCodeStr.includes(searchLower);

      // Role
      const matchesRole = roleFilter === "all" ||
        m.role === roleFilter ||
        (roleFilter === "LEADER" && (m.role === "LEADER" || m.role === "Leader")) ||
        (roleFilter === "MEMBER" && (m.role === "MEMBER" || m.role === "Member"));
  ...
  const handleAssignLeaderConfirm = () => {
    ...
    setMembers(prev => prev.map(m => {
      ...
      if (m.groupId === assignLeaderMember.groupId) {
        ...
        } else if (m.role === "LEADER" || m.role === "Leader") {
